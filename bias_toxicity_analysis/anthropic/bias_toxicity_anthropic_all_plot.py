...
def process_chosen(df, split):
    ...
    df2 = df1[df1['parent_categories'] != 'None']
    ...
    all_counts = df2.parent_categories.explode().fillna('None').value_counts()

    df2 = df2[df2['chosen_toxic'] == True]
    counts = df2.parent_categories.explode().fillna('None').value_counts()
    ...
    plt.xticks(rotation=90)
    plt.title(f"Anthropic Chosen Response Toxic Bias Distribution - Categories {split}")
    ax.barh(x, y, height = 0.8, color='mediumseagreen')
    for i, v in enumerate(total_percentages):
        if PERCENT_OF_TOXIC == True:
            ax.text(1.5 , i, f'{v:0.3f}% toxic of total',  fontsize=14, ha='left', va='center',color='black')
        else:
            ax.text(1.5 , i, f'{v:0.3f}% of total', fontsize=14, ha='left', va='center',color='black')

    plt.ylabel('Race')
    plt.xlabel('% Toxic of Race Category')

    plt.savefig(f"chosen_response_toxic_bias_distribution_categories_{split}.jpg", bbox_inches = 'tight')
    plt.show()
    plt.close()
    
    
    df2 = df1[df1['category'] != 'None']
    
    # plot race distribution plot
    all_counts = df2.category.explode().fillna('None').value_counts()

    df2 = df2[df2['chosen_toxic'] == True]
    counts = df2.category.explode().fillna('None').value_counts()
    ...
    plt.xticks(rotation=90)
    plt.title(f"Anthropic Chosen Response Toxic Bias Distribution - Subcategories {split}")
    ax.barh(x, y, height = 0.8, color='mediumseagreen')
    for i, v in enumerate(total_percentages):
        if PERCENT_OF_TOXIC == True:
            ax.text(1.5 , i, f'{v:0.3f}% toxic of total', fontsize=10, ha='left', va='center',color='black')
        else:
            ax.text(1.5 , i, f'{v:0.3f}% of total',  fontsize=10, ha='left', va='center',color='black')

    plt.ylabel('Race')
    plt.xlabel('% Toxic of Race Category')

    plt.savefig(f"chosen_response_toxic_bias_distribution_subcategories_{split}.jpg", bbox_inches = 'tight')
    plt.show()
    plt.close()
...
def process_rejected(df, split):
    ...
    all_counts = df2.parent_categories.explode().fillna('None').value_counts()
    df2 = df2[df2['rejected_toxic'] == True]
    counts = df2.parent_categories.explode().fillna('None').value_counts()
    ...
    fig, ax = plt.subplots(figsize=(10,15))
    plt.xticks(rotation=90)
    plt.title(f"Anthropic Rejected Response Toxic Bias Distribution - Subcategories - {split}")
    ax.barh(x, y, height = 0.8, color='mediumseagreen')
    ...
    plt.savefig(f"rejected_response_toxic_bias_distribution_subcategories_{split}.jpg", bbox_inches = 'tight')
    plt.show()
    plt.close()
